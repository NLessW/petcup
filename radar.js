...
class RadarSensor {
    ...
    calculateCRC16(buffer) {
        ...
        // Modbus RTU CRC는 Little Endian (Low Byte, High Byte)
        // 비교를 위해 Low | (High << 8) 형태로 반환
        return crc;
    }
    ...
    parseModbusResponse() {
        ...
        while (this.readBuffer.length >= 7) {
            ...
            for (let i = 0; i < this.readBuffer.length - 1; i++) {
                if (this.readBuffer[i] === 0x01 && this.readBuffer[i + 1] === 0x03) {
                    headerIndex = i;
                    break;
                }
            }
            ...
            if (this.readBuffer[2] === 0x02) {
                ...
                this.log(
                    `CRC 검증: 수신=0x${receivedCRC.toString(16).padStart(4, '0')}, 계산=0x${calculatedCRC.toString(16).padStart(4, '0')}`,
                    'info',
                );
                ...
                if (receivedCRC === calculatedCRC) {
                    ...
                    // 퍼센트 계산 (575mm 기준)
                    const fillPercent = distance >= 575 ? 0 : Math.round((1 - distance / 575) * 100);
                    this.log(`수거율: ${fillPercent}%`, 'info');
                } else {
                    this.log(
                        `CRC 오류! 수신=0x${receivedCRC.toString(16).padStart(4, '0')}, 계산=0x${calculatedCRC.toString(16).padStart(4, '0')}`,
                        'error',
                    );
                }
    ...
    drawContainer() {
        ...
        if (fillHeight > 0) {
            ...
            this.ctx.fillRect(containerX, containerY + containerHeight - fillHeight, containerWidth, fillHeight);
            ...
            for (let x = 0; x <= containerWidth; x += 5) {
                const y = waveY + Math.sin(x / 50 + time * waveFrequency) * waveAmplitude;
                if (x === 0) {
